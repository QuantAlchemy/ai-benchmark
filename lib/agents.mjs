...
function readCursorModels(path) {
  if (!path) return [];
  const result = spawnSync(path, ["--list-models"], {
    encoding: "utf8",
    timeout: 5000,
    env: process.env,
  });
  ...
  const parsed = output
    ...
    .map((match) => ({
      value: match[1],
      label: match[2],
      description: "Cursor account model.",
      isExactModel: true,
    }));
  const seen = new Set();
  return [
...
function readTopLevelTomlString(path, key) {
  ...
  try {
    ...
    // A missing or temporarily incomplete config should not hide the agent.
  }
  return "";
}

function readCodexCatalog(env = process.env) {
  const path = join(codexHome(env), "models_cache.json");
  if (!existsSync(path)) return [];
  try {
...
function configuredCodexModel(env = process.env) {
  const globalModel = readTopLevelTomlString(join(codexHome(env), "config.toml"), "model");
  const projectModel = readTopLevelTomlString(join(ROOT, ".codex", "config.toml"), "model");
  return projectModel || globalModel;
}

function resolveCodexDefaultModel(env = process.env, catalog = readCodexCatalog(env)) {
  return configuredCodexModel(env) || catalog[0]?.slug || "";
}

function reasoningLabel(effort) {
  const labels = { low: "Low", medium: "Medium", high: "High", xhigh: "Extra High", max: "Max", ultra: "Ultra" };
  return labels[effort] ?? String(effort);
}

function codexModelOption(model) {
  ...
  const supportsServiceTier =
    ...
    (Array.isArray(model.additional_speed_tiers) && model.additional_speed_tiers.length > 0);
  return {
    value: model.slug,
    label: model.display_name || model.slug,
    description: model.description || "Codex account model.",
    supportsServiceTier,
    supportsFastMode: supportsServiceTier,
    reasoningEfforts,
...
function readCodexModels(env = process.env) {
  const catalog = readCodexCatalog(env);
  if (!catalog.length) return [];
  const models = catalog.map(codexModelOption);
  const defaultModel = resolveCodexDefaultModel(env, catalog);
  const defaultOption = models.find((model) => model.value === defaultModel);
  return [
    {
      value: "",
      label: defaultOption ? `CLI default (${defaultOption.label})` : `CLI default (${defaultModel})`,
      description: defaultModel
        ? `Use the local Codex default, currently ${defaultModel}. The resolved model is recorded with the run.`
        : "Use the local Codex default. The resolved model is recorded when available.",
      resolvedModel: defaultModel,
      supportsServiceTier: Boolean(defaultOption?.supportsServiceTier),
      supportsFastMode: Boolean(defaultOption?.supportsFastMode),
      reasoningEfforts: defaultOption?.reasoningEfforts ?? [],
      defaultReasoningEffort: defaultOption?.defaultReasoningEffort ?? "",
    },
    ...models,
  ];
}

function readAgentModels(definition, executable) {
  if (!executable) return [];
  if (definition.id === "codex") return readCodexModels();
  if (definition.id === "cursor") return readCursorModels(executable.path);
  return [];
}
...
export function listAgents(env = process.env) {
  return AGENT_DEFINITIONS.map((definition) => {
    ...
    return {
      ...
      models: runnable ? readAgentModels(definition, executable) : [],
...
function buildInvocation(definition, input) {
  ...
  switch (definition.id) {
    ...
    case "cursor":
      ...
      if (input.model) {
        args.push("--model", formatCursorModel(input.model, { reasoningEffort, fastMode, knownModels: input.knownModels }));
      }
...
function formatCursorModel(model, options) {
  ...
  if (!baseModel || baseModel.includes("[") || options.knownModels?.has(baseModel)) return baseModel;
...
export async function runAgentOnBenchmark(bench, options = {}) {
  ...
  const resolvedModel = model || (definition.id === "codex" ? resolveCodexDefaultModel() : "");
  ...
  const knownModels =
    definition.id === "cursor"
      ? new Set(readCursorModels(executable.path).map((cursorModel) => cursorModel.value).filter(Boolean))
      : null;
  const invocation = buildInvocation(definition, { prompt, model, reasoningEffort, serviceTier, fastMode, knownModels });
