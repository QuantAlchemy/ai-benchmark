#!/usr/bin/env bash
# Smoke-test a candidate Cruiser Reloaded build. Objective gate: installs, builds, and
# produces a servable web app. Fun / completeness are judged by hand (RUBRIC.md).
#
...
if [[ -n "$OUTDIR" ]]; then
  ...
  assets=$(find "$OUTDIR" -type f \( -name '*.js' -o -name '*.png' -o -name '*.jpg' \
            -o -name '*.mp3' -o -name '*.ogg' -o -name '*.wav' -o -name '*.webp' \) \
            | wc -l | tr -d ' ')
...
if [[ -z "$SERVE" ]]; then
  ...
  ( cd "$SOL" && "$PM" run "$SERVE" ) >/tmp/cruiser_reloaded_serve.log 2>&1 &
  SRV_PID=$!
  ok_http=0
  ...
  [[ $ok_http -eq 0 ]] && note "! could not confirm a local server (see /tmp/cruiser_reloaded_serve.log) — verify manually"
fi

echo
echo "PASS: installs and builds. Now grade fun & completeness with: bench score cruiser-reloaded"
exit 0
