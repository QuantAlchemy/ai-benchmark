#!/usr/bin/env bash
# Smoke-test a candidate Voidbreaker build. Objective gate: installs, builds, and
...
if [[ -z "$SERVE" ]]; then
  ...
  ( cd "$SOL" && "$PM" run "$SERVE" ) >/tmp/voidbreaker_serve.log 2>&1 &
  SRV_PID=$!
  ok_http=0
  ...
  [[ $ok_http -eq 0 ]] && note "! could not confirm a local server (see /tmp/voidbreaker_serve.log) — verify manually"
fi

echo
echo "PASS: installs and builds. Now grade fun & completeness with: bench score voidbreaker"
exit 0
