...
describe("synchronization status presentation", () => {
  it("distinguishes an unconfigured, never-synced, failed, queued, and healthy installation", () => {
    ...
    expect(getSyncPresentation({ configured: true, lastSyncAt: null, lastError: null, failedOperations: 1, deadLetteredOperations: 1, pendingOperations: 0 })).toEqual({ label: "1 dead-lettered", variant: "warning" });
